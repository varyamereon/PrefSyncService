...
namespace PrideWearCompanion.Config
{
    ...
    public class PrefSyncService : WearableListenerService
    {
        ...
        public class PrefListener : Java.Lang.Object, ISharedPreferencesOnSharedPreferenceChangeListener
        {
            ...
            /// <summary>
            /// If you want to synchronize a specific <see cref="ISharedPreferences"/> file (rather than the app's
        ...
        private class PrefHandler : Handler, GoogleApiClient.IConnectionCallbacks
        {
            #region Fields
                    ...
                    if (msg.What == ACTION_SYNC_ALL)
                    {
                        ...
                        try
                        {
                            if (dataItems.Status.IsSuccess)
                            {
                                ...
                                {
                                    if (((Android.Gms.Common.Data.Zzc)dataItems.Get(i)).JavaCast<IDataItem>() is IDataItem dataItem)
                                    {
                                        Log.Debug(nameof(PrefSyncService), $"Resync {nameof(HandleMessage)}: {dataItem}");
                                        SaveItem(dataItem, editor, null);
                                    }
                                    else
                                    {
                                        // Invalid cast
